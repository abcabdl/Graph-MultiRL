...
from graphcredit_offline.integration import apply_graphcredit_rewards
...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, multi_turn=False, norm_adv_by_std_in_grpo=True, group_by_agent_id=False, step_advantage_w=1.0, gigpo_mode="mean_std_norm", gigpo_enable_similarity=False, gigpo_similarity_thresh=0.95, **kwargs):
    ...
    graphcredit_config = data.meta_info.get("graphcredit_config", None)
    if graphcredit_config and graphcredit_config.get("enabled", False):
        norm_mode = graphcredit_config.get("normalization", {}).get("mode", "agent_node_type")
        group_index = build_graphcredit_group_index(data, mode=norm_mode)
    elif group_by_agent_id:
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        graphcredit_reward_tensor, graphcredit_extra_infos, graphcredit_metrics = apply_graphcredit_rewards(
                            batch,
                            reward_tensor,
                            self.config,
                        ...
                        if graphcredit_extra_infos:
                            reward_tensor = graphcredit_reward_tensor
                            batch.batch["token_level_scores"] = reward_tensor
                            batch.non_tensor_batch.update({k: np.array(v, dtype=object) for k, v in graphcredit_extra_infos.items()})
                            metrics.update(graphcredit_metrics)
                        ...
                        batch.meta_info["graphcredit_config"] = dict(self.config.get("graphcredit", {}))
